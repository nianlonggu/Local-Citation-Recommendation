...
if __name__ == "__main__":
    ...
    val_corpus = json.load(open(args.val_corpus_path))
    ...
    train_dataloader = PrefetchLoader( train_dataset, batch_size= args.n_query_per_batch, shuffle= True, 
                                  worker_init_fn = lambda x:[np.random.seed( int( time.time() )+x ), torch.manual_seed(int( time.time() ) + x) ],
                                  num_workers= args.num_workers,  drop_last= True, 
                                  pin_memory= True )

    val_dataset = PrefetchDataset( val_corpus, paper_database, context_database, available_paper_ids, 
                                     args.max_seq_len, 
                                     args.max_doc_len, 
                                     words, 
                                     max_n_sample = args.max_n_sample,
    ...
    val_dataloader = PrefetchLoader( val_dataset, batch_size= args.n_query_per_batch, shuffle= True, 
                                  worker_init_fn = lambda x:[np.random.seed( int( time.time() )+x ), torch.manual_seed(int( time.time() ) + x) ],
                                  num_workers= args.num_workers,  drop_last= True, 
                                  pin_memory= True )
    ...
    for count in tqdm(range( args.max_num_iterations)):
        ...
        if  args.validate_every is not None and current_batch % args.validate_every == 0:
            running_losses_val = []
            for _ in range( args.num_validation_iterations ):
