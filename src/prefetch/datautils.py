...
import json
import numpy as np
from metrics import JaccardSim
...
class PrefetchDataset( Dataset ):
    def __init__(self, corpus= [], 
        ...
        if available_paper_ids is not None:
            ...
            self.available_paper_ids = available_paper_ids
        #self.paper_id_to_pos_mapper = dict( list(zip( self.available_paper_ids, np.arange(len(self.available_paper_ids)).tolist() )) )
    ...
    def __getitem__(self, _ ):
        ...
        while True:
            ...
            if self.cr_mode == "local":
                ...
                citing_id = corpus_item["id"]
                document_info.append( self.encode_document( self.load_document( citing_id, is_citing_document = True  ) ) )
                query_text =  " ".join([ _[0] for _ in self.load_document( citing_id, is_citing_document = True  ) ])
            ...
            low_thres_jaccard_sim = jaccard_sim_of_positive_ids[int( len( jaccard_sim_of_positive_ids ) * 0.1 )]
            up_thres_jaccard_sim = jaccard_sim_of_positive_ids[int( len( jaccard_sim_of_positive_ids ) * 0.9 )]
            ...
            for pos in np.random.choice( len(hard_negative_ids), len(hard_negative_ids), replace = False ):
                ...
                if hard_negative_jaccard_sim <  avg_thres_jaccard_sim:   # low_thres_jaccard_sim  :
                    ...
                    if pos_count >= self.max_n_hard_negative:
                        break
                elif hard_negative_jaccard_sim > avg_thres_jaccard_sim:  # up_thres_jaccard_sim  :
                    document_info.append( self.encode_document( document_text ) )
                    class_label_list.append( idx )
